...
class DatabaseServiceTests(unittest.TestCase):
    ...
    def test_classify_query(self) -> None:
        self.assertEqual(self.service.classify_query("SELECT 1"), "read")
        self.assertEqual(self.service.classify_query("WITH cte AS (SELECT 1) SELECT * FROM cte"), "read")
        self.assertEqual(self.service.classify_query("PRAGMA table_info(users)"), "read")
        self.assertEqual(self.service.classify_query("EXPLAIN SELECT 1"), "read")
        self.assertEqual(self.service.classify_query("INSERT INTO users VALUES (1,'a',1)"), "dml")
        self.assertEqual(self.service.classify_query("UPDATE users SET age=1"), "dml")
        self.assertEqual(self.service.classify_query("DELETE FROM users"), "dml")
        self.assertEqual(self.service.classify_query("REPLACE INTO users VALUES (1,'a',1)"), "dml")
        self.assertEqual(self.service.classify_query("CREATE TABLE t (id INTEGER)"), "ddl")
        self.assertEqual(self.service.classify_query("ALTER TABLE users ADD COLUMN email TEXT"), "ddl")
        self.assertEqual(self.service.classify_query("DROP TABLE users"), "ddl")
        self.assertEqual(self.service.classify_query("BEGIN"), "tcl")
        self.assertEqual(self.service.classify_query("COMMIT"), "tcl")
        self.assertEqual(self.service.classify_query("ROLLBACK"), "tcl")
        self.assertEqual(self.service.classify_query("-- comment\nSELECT 1"), "read")
